...
while page_num <= final_num:
	...
	page = urlopen("http://jisho.org/api/v1/search/words?keyword=%23chem&page=" + str(page_num))
...
# For each element in each list within the list, add a number: 1, 2, 3, etc.
		...
		while num != len(words['senses']):
			...
			if "Chemistry term" in word_tags:
...
filename = "chemistry-scrape-output.txt" # The output file name.
