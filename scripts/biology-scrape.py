...
while page_num <= final_num:
	...
	page = urlopen("http://jisho.org/api/v1/search/words?keyword=%23biol&page=" + str(page_num))
...
filename = "biology-scrape-output-error.txt" # The output file name.
...
while num2 < len(kanji_list):
	# If the entry has a kanji, then print that as well as the hiragana. Otherwise, just print the hiragana.
	if kanji_list[num2] != "":
	...
	print(parent_eng_list[num2][-1][:-1], file = open(filename, "a"))
	num2 += 1